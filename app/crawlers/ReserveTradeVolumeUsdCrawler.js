...
const getUnprocessedTrades        = require("./getUnprocessedReserveTrade")
...
class ReserveTradeVolumeUsdCrawler {
  ...
  start () {
    async.auto({
      ...
      unprocessedTrades: ['config', (ret, next) => {
        ...
        getUnprocessedTrades(next, "ReserveTradeModel", null, 'reserve_trade');
      }],
      processTrades: ['unprocessedTrades', (ret, next) => {
        // console.log("----------------   unprocessedTrades   -------------", ret)
  ...
  _processTradesOnce (exSession, packTrades, callback) {
    ...
    async.eachLimit(Object.keys(dateTrade), PARALLEL_INSERT_LIMIT, (date, asyncCallback) => {
      ...
      const timeInMillis = new Date(dateSplit[2], dateSplit[1] - 1, dateSplit[0]).getTime()
  ...
  _updateTradePrice(exSession, packTrades, datePrice, callback){
    ...
    packTrades.map(t => {
      updateRows.push({
        id: t.id,
        valueUsd: t.value_eth * datePrice[t.date].usdPrice,
    ...
    async.waterfall([
      (next) => {
        async.eachLimit(updateRows, PARALLEL_INSERT_LIMIT, (record, _next) => {
          ReserveTradeModel.update(record, _next);
...
module.exports = ReserveTradeVolumeUsdCrawler;
