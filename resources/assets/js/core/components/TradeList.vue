        ...
        <table class="table table-hover table-responsive table-round">
          <thead>
            <tr>
              ...
              <th class="text-right">{{ $t("trade_list.collected_fees") }}</th>
              <th></th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(row, index) in rows" :item="row" :index="index" @click="onClickRow(row)">
              ...
              <td v-if="!isHidePartnerCommission"  class="text-right">{{ formatTokenNumber('KNC', row.commission) }} KNC</td>
              <td class="text-right no-padding-right">{{ formatTokenNumber('KNC', row.collectedFees) }} KNC</td>
