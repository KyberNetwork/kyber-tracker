...
|official|BOOL|NO| true or false (default is true). If `false` will return volume of all tokens (offical and unoffical trades), if `true` will return only offical|
...
<!-- |official|BOOL|NO| true or false (default is true). If `false` will return all trades of all tokens (offical and unoffical trades), if `true` will return only offical token pair| -->
